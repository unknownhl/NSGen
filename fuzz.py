...
class Parameters(object):
    def __init__(self, base_args):
        ...
        self.save_every = 100
        self.output_dir = f'./Fuzzer/{self.dataset}-{self.model}/'
        ...
        contrast = list(
            itertools.product([getattr(image_transforms, "image_contrast")], [0.8 + 0.2 * k for k in range(7)]))
        brightness = list(
            itertools.product([getattr(image_transforms, "image_brightness")], [10 + 10 * k for k in range(7)]))
        blur = list(itertools.product([getattr(image_transforms, "image_blur")], [k + 1 for k in range(10)]))

        self.stylized = Stylized(self.image_size,self.device)
    ...
    def run(self, I_input, L_input):
        ...
        while not self.can_terminate():
            ...
            for s_i in range(len(S)):
                ...
                for i in range(1, Ps(s_i) + 1):
                    ...
                    if self.isFailedTest(I_new):
                        ...
                        elif self.params.criterion == 'LG':
                            old_torch_image = self.image_to_input(np.concatenate((B_old, [I])))
                            old_torch_image = old_torch_image.to(self.params.device)
                            cov_dict = self.criterion.calculate(old_torch_image, torch_image, torch_label)
                        else:
            ...
            if len(B_new) > 0:
                ...
                num_wrong, ae_index, inx = self.is_adversarial(new_image, new_label)
                if num_wrong > 0:
                    self.num_ae += num_wrong

                if self.epoch % self.params.save_every == 0:
                    self.saveImage(B_new / self.params.input_scale,
                                   self.params.image_dir + (f'%03d_new.jpg' % self.epoch)
                                   )
                    self.saveImage(B_old / self.params.input_scale,
                                   self.params.image_dir + ('%03d_old.jpg' % self.epoch)
                                   )
                    if num_wrong > 0:
                        save_image(new_image[ae_index].data,
                                   self.params.image_dir + ('%03d_ae.jpg' % self.epoch),normalize=True)

            gc.collect()

            if self.epoch % 50==0:
                print(f'The current epoch is {self.epoch}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--criterion', type=str, default='LG',
                        choices=['LG', 'NLC', 'NC', 'KMNC', 'SNAC', 'NBC', 'TKNC', 'TKNP', 'CC',
                                 'LSC', 'DSC', 'MDSC','LG'])
    parser.add_argument('--ab_exp', type=str, default='clip')
    parser.add_argument('--device', type=str, default='cuda:3')
    base_args = parser.parse_args()

    args = Parameters(base_args)

    if args.criterion == 'LG':
        args.exp_name = ('%s-%s-%s-%s' % (args.dataset, args.model, args.criterion, str(args.ab_exp)))
    ...
    if args.criterion == 'LG':
        pre_mutate_dataset = data_loader.PreMutateDataset(
            torch.from_numpy(np.array(image_numpy_list)).permute(0, 3, 1, 2),
            torch.from_numpy(np.array(label_numpy_list)), args)

        if args.model == 'vgg16_bn':
            LAYERS = tuple(f'features.{index}' for index in (3, 10, 20, 30, 40))
        elif args.model == 'resnet50':
            LAYERS = ('conv1', 'layer1', 'layer2', 'layer3', 'layer4')
        elif args.model == 'mobilenet_v2':
            LAYERS = tuple(f'features.{index}' for index in range(0, 19, 2))

        results_dir = f'./outputs/exemplars/{args.model}/{args.dataset}/'
        if not os.path.exists(results_dir):
            os.makedirs(results_dir)
        if len(os.listdir(results_dir)) == 0:
            for layer in LAYERS:
                compute.discriminative(model, pre_mutate_dataset, layer=layer, units=None, results_dir=results_dir,
                                       viz_dir=None,
                                       save_viz=False,
                                       device=args.device,
                                       num_workers=16)
    ...
    if args.use_sc:
        criterion = getattr(coverage, args.criterion)(model, layer_size_dict,args.device, hyper=hyper_map[args.criterion],
                                                      min_var=1e-5, num_class=TOTAL_CLASS_NUM)
    elif args.criterion == 'LG':
        criterion = getattr(ns, args.criterion)(model, args.model, args.dataset,args.device,results_dir, args.ab_exp,
                                                      layer_name=LAYERS)
    else:
        criterion = getattr(coverage, args.criterion)(model, layer_size_dict,args.device, hyper=hyper_map[args.criterion])

    criterion.build(train_loader)
    if args.criterion not in ['LG', 'CC', 'TKNP', 'LSC', 'DSC', 'MDSC']:
