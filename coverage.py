...
class Coverage:
    def __init__(self, model, layer_size_dict, device, hyper=None, **kwargs):
        self.device = device
        self.model = model
    ...
    def calculate(self, data_batch, label_batch):
        ...
        for i, label in enumerate(label_batch):
            ...
            if self.num_class <= 1:
                ...
                lsa = np.ndarray.item(-self.kde_cache[int(label.cpu())].logpdf(np.expand_dims(SA, 1)))
            else:
                # lsa = np.asscalar(-self.kde_cache[int(label.cpu())].score_samples(np.expand_dims(SA, 0)))
                lsa = np.ndarray.item(-self.kde_cache[int(label.cpu())].score_samples(np.expand_dims(SA, 0)))
...
class MDSC(SurpriseCoverage):
    ...
    def set_mask(self):
        ...
        for layer_name in self.mean_dict.keys():
            ...
            feature_num += self.mask_index_dict[layer_name].size(0)
        print('feature_num: ', feature_num)
        self.estimator = tool.Estimator(feature_num=feature_num,device=self.device, num_class=self.num_class)
        ...
        for (layer_name, layer_output) in layer_output_dict.items():
            ...
            for single_output in layer_output:
                ...
                if len(self.distant_dict[layer_name]) > 0:
                    _, approx_distances = self.flann_dict[layer_name].nn_index(np.expand_dims(single_output, 0),
                                                                               num_neighbors=1)
                    exact_distances = [
