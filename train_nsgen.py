...
parser.add_argument('--embedding_size', type=int, help='Size of input word embeddings (default: 128)')
parser.add_argument('--hidden_size', type=int, help='Size of hidden state (default: 512)')
args = parser.parse_args()

device = args.device or 'cuda:1' if cuda.is_available() else 'cpu'
...
lm = None
if not args.no_lm:
    lm_file = results_dir / f'lm_{args.embedding_size}_{args.hidden_size}.pth'
    if lm_file.exists():
        print(f'loading cached lm from {lm_file}')
        lm = milan.LanguageModel.load(lm_file, map_location=device)
        lm.eval()
    else:
        lm = milan.lm(dataset, embedding_size=args.embedding_size, hidden_size=args.hidden_size)
...
decoder_file = results_dir / f'decoder_{args.embedding_size}_{args.hidden_size}.pth'
if decoder_file.exists():
    ...
    decoder = milan.decoder(dataset, encoder, lm=lm, embedding_size=args.embedding_size, hidden_size=args.hidden_size)
    decoder.fit(dataset,
                features=features,
